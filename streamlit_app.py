...
def main():
    ...
    if country_option == 'Norway':
        ...
        fig = px.scatter_mapbox(norway_data, lat="lat", lon="lon", hover_name="kommune", hover_data=['main_owner','year_commissioned','operating_turbines'],
        ...
        fig = px.scatter_mapbox(all_data, lat="lat", lon="lon", hover_name="county", hover_data=['case_id'],
                            size_max = 10, zoom=3, height=800, widht = 800 ) #color_continuous_scale = px.colors.sequential.Magenta color = 'avg_annual_prodcution_GWH',size = 'year_commissioned',
        fig.update_layout(mapbox_style="open-street-map")
        fig.update_layout(margin={"r":0,"t":0,"l":0,"b":0})
        fig.show()
